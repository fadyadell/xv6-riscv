...
#include "pstat.h"
#define NPROC 64
struct proc proc[NPROC];      // Declare the process table array
// extern struct spinlock ptable.lock;  // Correct lock name (guessing from context)
...
{
  struct proc *p = myproc();
  p->end_time = ticks; // <-- Add this line to record process completion time
...
scheduler(void)
{
  ...
  for(;;){
    intr_on();
    int found = 0;
    struct proc *p = choose_next_process();
    if(p != 0) {
      acquire(&p->lock);
      if (p->state == RUNNABLE) {
        p->state = RUNNING;
...
int
getptable(int nproc, struct pstat *ps)
{
    struct proc *p;
    struct pstat local_ps[NPROC];
    int i = 0;
    ...
    for(p = proc; p < &proc[NPROC] && i < nproc; p++) {
        acquire(&p->lock);
        if(p->state != UNUSED) {
            local_ps[i].pid = p->pid;
            local_ps[i].ppid = p->parent ? p->parent->pid : -1;

            // Copy state string
            switch(p->state) {
                case UNUSED:   safestrcpy(local_ps[i].state, "UNUSED",   sizeof(local_ps[i].state)); break;
                case SLEEPING: safestrcpy(local_ps[i].state, "SLEEPING", sizeof(local_ps[i].state)); break;
                case RUNNABLE: safestrcpy(local_ps[i].state, "RUNNABLE", sizeof(local_ps[i].state)); break;
                case RUNNING:  safestrcpy(local_ps[i].state, "RUNNING",  sizeof(local_ps[i].state)); break;
                case ZOMBIE:   safestrcpy(local_ps[i].state, "ZOMBIE",   sizeof(local_ps[i].state)); break;
...
// ZOMBIE: Process has exited but parent hasn't waited for it
    ...
    // Copy to user space
    if(copyout(myproc()->pagetable, (uint64)ps, (char *)local_ps, sizeof(struct pstat) * i) < 0)
        return -1;
